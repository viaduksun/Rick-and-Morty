...
const EpisodeField = ({ episodes }) => {
  ...
  if (episodes) {
    emptyRows =
      rowsPerPage - Math.min(rowsPerPage, episodes.length - page * rowsPerPage);
  }
                ...
                {stableSort(episodes, getComparator(order, orderBy))
                  ...
                  .map((episode, index) => {
                    ...
                    return (
                      <StyledTableRow
                        hover
                        onClick={(event) => handleClick(event, episode.name)}
                        role="checkbox"
...
export default memo(EpisodeField);
